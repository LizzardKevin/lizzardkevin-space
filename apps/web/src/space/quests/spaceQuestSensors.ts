...
function createWorkGazeSensor(): SpaceQuestSensor {
  ...
  return (event, nowMs) => {
    ...
    if (event.type === "pose-sampled") {
      // ExhibitRaycast 只在目标身份变化时通知;稳定注视期间用既有 ≤10Hz pose 流推进计时。
      return (
        targetId !== null &&
        gazeSince !== null &&
        lostAtMs === null &&
        nowMs - gazeSince >= WORK_GAZE_HOLD_MS
      );
    }
...
function createProjectorSlideSensor(): SpaceQuestSensor {
  return (event) => {
    // 事件源已经过滤挂载基线与无效按键;收到一次即代表画面真实切换。
    return event.type === "projector-slide-changed";
  };
}
