...
assert(files.mobileExperience.includes("applyTerminalScrollState"), "mobile scroll must share the measured chrome layout helper");
...
assert(files.mobileExperience.includes("--terminal-nav-scroll-y"), "mobile scroll must keep navigation transform ownership explicit during collapse");
...
assert(files.mobileExperience.includes("header.getBoundingClientRect().height"), "terminal layout must measure the actual header including safe-area padding");
assert(files.mobileExperience.includes("nav.getBoundingClientRect().height"), "terminal layout must measure navigation after font and row reflow");
assert(files.mobileExperience.includes('"--terminal-shell-top", `${headerHeight + navHeight}px`'), "the sole scroll shell must start below both measured chrome sections");
assert(files.mobileExperience.includes("const contentScrollY = Math.max(0, expandedHeaderHeight - headerHeight)"), "document movement must compensate actual header shrink without a fixed shell offset");
assert(files.mobileExperience.includes("useLayoutEffect"), "terminal chrome geometry must be initialized before paint");
assert(files.mobileExperience.includes("new ResizeObserver(updateLayout)"), "terminal layout must react to chrome size changes");
assert(files.mobileExperience.includes("observer.observe(header)") && files.mobileExperience.includes("observer.observe(nav)"), "layout observation must cover both header and navigation");
assert(files.mobileExperience.includes("observer.disconnect()"), "terminal layout must release its size observer on cleanup");
