...
assert.match(overlay, /reportVideoMetadata\(videoUrl,\s*["']loaded["']\)/);
assert.match(overlay, /reportVideoMetadata\(videoUrl,\s*["']failed["']\)/);
assert.match(overlay, /selectedMediaSessionRef\.current\?\.cancel\(\);\s*setSelectedVideoMetadataSnapshot\(null\)/);
