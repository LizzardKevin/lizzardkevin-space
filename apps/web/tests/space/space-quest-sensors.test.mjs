...
test("dont_look_away: 目标只在进入时通知一次,持续注视由 pose 采样推进完成", () => {
  const sensor = createSpaceQuestSensor("dont_look_away");
  let now = 1_000;
  sensor({ type: "work-targeted", exhibitId: "x" }, now);
...
test("next_scene: 事件源过滤初始画面后,第一次真实切换即完成", () => {
  ...
  assert.equal(
    sensor({ type: "projector-slide-changed", slideId: "s2" }, 1_000),
    true,
    "SpaceProjectorInstallation 只会在画面已实际切换后投递该事件",
  );
});
