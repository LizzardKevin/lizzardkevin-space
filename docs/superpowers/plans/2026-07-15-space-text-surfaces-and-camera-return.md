...
- [ ] **Step 5: Capture StartLobby and in-SPACE hint screenshots**
...
- [ ] **Step 1: Write a failing stable-listener contract**
...
- [ ] **Step 3: Implement stable listener ownership**

Render `GuardedPointerLockControls` unconditionally inside `SpaceScene`, pass `enabled={pointerControlsEnabled}`, store `enabled` in a ref updated by layout effect, and keep pointerlock/mousemove/click listeners mounted. Begin `onMouseMove` and `lock` with an enabled-ref guard. Preserve warmup filtering, raw fallback, event compute restoration, camera math, and cleanup.
...
- [ ] **Step 5: Prove actual camera motion in production browser QA**
...
- [ ] **Step 6: Commit the interaction fix**
