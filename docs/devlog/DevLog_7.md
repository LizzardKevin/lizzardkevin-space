...
新 `space_main.glb` 的节点已经能在 Blender / GLB 里查到，这轮改看网页里到底能不能正常走。重点是查清“局部速度忽快忽慢”来自模型碰撞、角色控制器，还是运行时 timestep。

为此先留一个临时 debug 面板，在真实浏览器里走动时直接看速度、位置和当前接触的 `COL_*`。确认原因后，修掉第一人称移动混用动态 render `dt` 带来的体感漂移，再把走路/冲刺速度调回之前确认的基线。
...
- overlay 挂在 `SpaceDesktopExperience`，用于真实 Chrome / Edge 手测。
...
- 我试过用自定义 guarded pointer lock controls 过滤异常 `movementY`，想处理视角偶发瞬间看天/看地的问题。
- 实测后第一人称视角完全不能移动，所以退回上一版 Drei `PointerLockControls`：
  - 先恢复可用性。
  - 视角跳闪以后在 Drei 控制链路上做小范围防护，不再整套替换 controls。

### 4) 固定 Physics Tick

- 排查中一度怀疑 Vite dev server 或“服务器 tick”改变了移动手感。实际 Vite server 只负责 dev server / HMR，不驱动 SPACE 物理 tick。问题出在浏览器运行时：
...
- in-app Browser 自动化仍因当前 Codex 环境的 `sandboxPolicy` 元数据错误无法连接，WebGPU / pointer lock 继续用真实浏览器手测。

## 下一步计划

1. 刷新 `5173`，用 debug overlay 观察 fixed tick 后速度、ratio、contact 是否稳定。
2. 保留 Drei `PointerLockControls`，在不替换整套 controls 的前提下处理 pointer lock 重进/焦点切换时的异常 delta。
3. 如果某个区域仍卡顿，记录 overlay 的 `contact` 名称，回到 Blender 中检查对应 `COL_*` 是否重叠、未 Apply Scale 或法线/面片过碎。
4. debug overlay 短期留在 dev 环境；上线前删除，或者改成隐藏的开发开关。
5. 继续整理 `space_main` 资产规范，让可见结构、碰撞代理、spawn marker、AO bake 和材质脚本保持为单一可重复流程。
